...
function getLocalDate(date, isSeconds, isISO) {
  ...
  const fulldateISO = `${year}-${month}-${day}, ${hour}:${minutes}:${seconds}`;
  const datewithoutsec = `${day}.${month}.${year}, ${hour}:${minutes}`;
  const datewithoutsecISO = `${year}-${month}-${day}, ${hour}:${minutes}`;
  let result;
  ...
  if (isISO && !isSeconds) {
    result = datewithoutsecISO;
  }

  if (!isISO && !isSeconds) {
    result = datewithoutsec;
  }
...
console.log(getWeekDay('2019-07-28')); // воскресенье
